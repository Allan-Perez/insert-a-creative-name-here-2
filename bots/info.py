        ...
        for gameObject in self.gameObjects:
            ...
            for key in gameObject.keys():
                if key == 'Id':
                    ...
                    convertedDictionary.append(((gameObject[
                                                     key] / 360) + 0.5) % 1)  # 0.5 == 0 degrees, 1 == 180 degrees, 0.25 = -90 degrees
    ...
    def wallDistance(posX, posY, heading, forward=True):
        ...
        for i in range((len(playground_points))):
            ...
            if intersect_pt == "none":
                continue
            if ((pl_line[0][0] <= intersect_pt[0] <= pl_line[1][0] or pl_line[0][0] >= intersect_pt[0] >= pl_line[1][
                0]) and (
                    pl_line[0][1] <= intersect_pt[1] <= pl_line[1][1] or pl_line[0][1] >= intersect_pt[1] >= pl_line[1][
                1])):
